    ...
    if (resend) {
      try {
        await resend.emails.send({
          from: MAIL_FROM,
          to: cart.email,
          subject: `${cart.snapshot.items[0]?.quantity ?? 1} × ${cart.snapshot.items[0]?.name ?? 'Ginger Fizz'}${cart.snapshot.items.length > 1 ? ` and ${cart.snapshot.items.length - 1} more` : ''} still in your cart`,
